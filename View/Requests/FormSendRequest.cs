    ...
    public partial class FormSendRequest : Form
    {
        ...
        private void SendEmail(string mailAddress, string formatFile)
        {
            ...
            if (formatFile == "xls")
            {
                try
                {
                    filePath = Path.GetFullPath("Заявка.xls");
                    service.SaveRequestToExcelFile(service.GetElement(id.Value), filePath);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            else if (formatFile == "doc")
            {
                try
                {
                    filePath = Path.GetFullPath("Заявка.doc");
                    service.SaveRequestToWordFile(service.GetElement(id.Value), filePath);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
